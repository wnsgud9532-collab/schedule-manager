...
def render():
    ...
    with st.container(border=True):
        ...
        st.caption("저장 경로: `data/schedule.db` (앱 재시작 시 초기화됨 → 구글 시트에 자동 백업/복원)")
    ...
    st.divider()
    with st.container(border=True):
        st.markdown("### ☁️ 구글 시트 백업")
        st.caption(
            "엑셀로 가져온 근무 데이터는 저장/수정할 때마다 구글 시트에 자동 백업되고, "
        ...
        if st.button("지금 수동 백업", key="settings_manual_backup"):
            try:
                ...
                st.error(f"백업 실패: {e}  \n(구글 시트에 서비스 계정이 '편집자'로 공유되어 있는지 확인하세요)")
